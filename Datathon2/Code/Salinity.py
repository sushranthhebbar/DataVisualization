...
with zipfile.ZipFile("../datathon2_data.zip") as z:
   with z.open("OneDrive_1_12-09-2020/Salinity_3D/111_02_Jul_2005.txt") as f:
...
mean_z_list=[]
for z in range(5,235,10):
    curr_df=df_z.get_group(z)
    curr_mean=curr_df['SALT'].mean()
...
ax.set_title('Mean of salinity vs depth',pad=20)
ax.set_xlabel('Salinity in (psu)',labelpad=10)
...
for k in tqdm(range(5,105,10)):
    ...
    surfcolor=df_z.get_group(k)['SALT'].tolist()
...
fig.update_layout(
         title='Slicing across z for salinity (in psu)',
         width=600,
         height=600,
         scene=dict(
         ...
         sliders=sliders,
         coloraxis=dict(colorscale='Haline',
...
offline.plot(fig, auto_open=True, image = 'png', image_filename="" ,image_width=2000, image_height=1000,
              filename='Media/Interactive/Salinity/salinity_z.html', validate=True)
...
ax.set_title('Mean of salinity vs longitude ',pad=20)
ax.set_xlabel('Longitude',labelpad=10)
# ax.xaxis.set_label_position('top')
ax.set_ylabel('Salinity in (psu)')
...
#     z=-k*np.ones((y.shape[0],x.shape[1]))
    surfcolor=df_x.get_group(lon[k])['SALT'].tolist()
    surfcolor=np.asarray(surfcolor)
    surfcolor=np.reshape(surfcolor,(z.shape))
...
surfcolor_start=df_x.get_group(51.25)['SALT'].tolist()
surfcolor_start=np.asarray(surfcolor_start)
surfcolor_start=np.reshape(surfcolor_start,(x_start.shape))
...
fig.update_layout(
         title='Slices across longitude for salinity(in psu)',
         width=600,
         height=600,
         scene=dict(
                    zaxis=dict(range=[-105, 0], autorange=False),
...
offline.plot(fig, auto_open=True, image = 'png', image_filename="" ,image_width=2000, image_height=1000,
              filename='Media/Interactive/Salinity/salinity_x.html', validate=True)
...
ax.set_title('Mean of salinity vs latitude ',pad=20)
ax.set_xlabel('Salinity in (psu)',labelpad=10)
# ax.xaxis.set_label_position('top')
...
#     z=-k*np.ones((y.shape[0],x.shape[1]))
    surfcolor=df_y.get_group(lat[k])['SALT'].tolist()
    surfcolor=np.asarray(surfcolor)
    surfcolor=np.reshape(surfcolor,(z.shape))
...
surfcolor_start=df_y.get_group(-29.7509)['SALT'].tolist()
surfcolor_start=np.asarray(surfcolor_start)
...
fig.update_layout(
         title='Slices across latitude for salinity(in psu)',
         width=600,
         height=600,
         scene=dict(
                    zaxis=dict(range=[-105, 0], autorange=False),
...
offline.plot(fig, auto_open=True, image = 'png', image_filename="" ,image_width=2000, image_height=1000,
            filename='Media/Interactive/Salinity/salinity_y.html', validate=True)
